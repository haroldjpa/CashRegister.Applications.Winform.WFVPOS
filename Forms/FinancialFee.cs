...
namespace CashRegister.Applications.Winform.WFVPos.Forms
{
  public partial class FinancialFee : Form
  {
      ...
      lock (_sync)
      {
        ...
        try
        {
          ...
          if (SYS_COUNTRY_APPLICATION.Honduras_Ficohsa == GlobalInformation.Instance.SysCountryApplication)
          {
            ...
            paymentParams.AcquireId = acquirerInfoSelected.id; //Ficohsa;
            paymentParams.Currency = acquirerInfoSelected.CurrencyID; //Lempira
    ...
    void FillAdquirerList(int indice)
    {
      ...
      foreach (var item in GlobalInformation.Instance.ListAcquirer)
      {
        if (comboListCount == indice)
        {
          ...
          acquirerInfoSelected.name = item.Value.name;
          acquirerInfoSelected.MerchantID = item.Value.MerchantID;
          acquirerInfoSelected.TerminalID = item.Value.TerminalID;
          acquirerInfoSelected.id = item.Value.id;
          acquirerInfoSelected.ListCuotas = item.Value.ListCuotas;
          acquirerInfoSelected.CurrencyID = item.Value.CurrencyID;
    ...
    private void CleanWidgets(int indice)
    {
